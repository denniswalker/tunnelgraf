...
def test_show_command(runner):
    ...
    result = runner.invoke(
        cli, ['--profile', 'tests/connections_profiles/four_in_a_row.yaml', 'show'])
...
def test_urls_command(runner):
    ...
    result = runner.invoke(
        cli, ['--profile', 'tests/connections_profiles/four_in_a_row.yaml', 'urls'])
    assert result.exit_code == 0
...
def test_scp_file_to(runner):
    """Test SCP copy of a file into bastion."""
    result = runner.invoke(cli, [
        '--profile',
        'tests/connections_profiles/four_in_a_row.yaml',
        'scp',
        'tests/test_file_transfer/test1',
        'bastion:/tmp/'
    ])
    # Note: This test would need a running tunnel to actually work
